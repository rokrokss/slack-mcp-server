...
const BINARY_MAP = {
    darwin_x64:   { name: '@rokrokss/slack-mcp-server-darwin-amd64',    suffix: '' },
    darwin_arm64: { name: '@rokrokss/slack-mcp-server-darwin-arm64',    suffix: '' },
    linux_x64:    { name: '@rokrokss/slack-mcp-server-linux-amd64',     suffix: '' },
    linux_arm64:  { name: '@rokrokss/slack-mcp-server-linux-arm64',     suffix: '' },
    win32_x64:    { name: '@rokrokss/slack-mcp-server-windows-amd64',   suffix: '.exe' },
    win32_arm64:  { name: '@rokrokss/slack-mcp-server-windows-arm64',   suffix: '.exe' },
};
...
function resolveBinaryPath() {
    ...
    if (process.env.SLACK_MCP_DXT) {
        ...
        // Remove scope from binary file name
        const binaryFileName = binary.name.replace('@rokrokss/', '');
        return require.resolve(`${binary.name}/bin/${binaryFileName}${binary.suffix}`);
    }
}
